...
export const getSession =
  (withLanguageAccount?: boolean): AppThunk =>
  async (dispatch, getState) => {
    await dispatch(getAccount());
    await dispatch(getLanguages(defaultPagination));

    const { account } = getState().authentication;
    if (account && account.languageId) {
      const languages = getState().language.entities || [];
      const language = languages.find(item => item.id === account.languageId)?.enumKey;
      const langKey = Storage.session.get('locale', language || 'pl');
      dispatch(setLocale(withLanguageAccount ? language : langKey));
    }
  };
