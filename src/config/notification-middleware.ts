...
export default ({ getState }) =>
  next =>
    ...
    const roleIds = state?.authentication?.account?.roleIds || [];
    const isAdmin = hasAnyAuthority(roleIds, [AUTHORITIES.ADMINISTRATOR.id]);
    const isSystem = hasAnyAuthority(roleIds, [AUTHORITIES.SYSTEM_PRO_ECO_MODULE.id]);

    /**
     *
     * The notification middleware serves to add success and error notifications
     */
    if ((isAdmin || isSystem) && isFulfilledAction(action) && payload && payload.headers) {
        ...
        if (
          ...
          switch (response.status) {
            ...
            case 400: {
              ...
              if (errorHeader) {
                if ((errorHeader !== 'error.emailExists' && response?.config?.url === 'api/register') || errorHeader) {
                ...
                for (const fieldError of fieldErrors) {
                  ...
                  const fieldName = translate(`proEcoApp.${fieldError.objectName}.${convertedField}`);
